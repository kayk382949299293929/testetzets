...
const ceemde = JSON.parse(fs.readFileSync('./lib/data/totalcmd.json'))
const dindin = JSON.parse(fs.readFileSync('./lib/usuarios/dindin.json'))
...
const getGlobalSpinner = () => {
  if(!globalSpinner) globalSpinner = new spin({ color: 'pink', succeedColor: 'pink', spinner});
...
const banner = cfonts.render(('AYUMI|BOT'), {
                font: 'block',
                color: '#FF00DB',
    align: 'center',
    gradient: ["#FF00DB","#FF00DB"],
    lineHeight: 3
  });

const banner2 = cfonts.render(('Sacole'), {
    font: 'chrome',
    align: 'center',
...
const banner3 = cfonts.render((`Ayumi-bot`), {
font: 'simple',
color: 'system',
align: 'center',
...
const addATM = (sender) => {
            ...
            dindin.push(obj)
            fs.writeFileSync('./lib/usuarios/dindin.json', JSON.stringify(dindin))
        }
...
const cmdadd = () => {
	ceemde[0].totalcmd += 1
	fs.writeFileSync('./lib/data/totalcmd.json', JSON.stringify(ceemde))
}

authorname = 'Ayumi-bot'
packname = 'Sacole'
  
function addMetadata(packname, author) {	
if (!packname) packname = 'Ayumi-bot'; if (!author) author = 'sacole';	
...
const daily = JSON.parse(fs.readFileSync('./lib/diario.json'))

const addLimit = (userId, _dir) => {
    const obj = { id: userId, time: Date.now() }
    _dir.push(obj)
    fs.writeFileSync('./lib/diario.json', JSON.stringify(_dir))
}
